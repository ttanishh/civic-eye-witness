...
export default function AdminReportsPage() {
      ...
      try {
        ...
        const mockReports = Array.from({ length: 15 }, (_, i) => {
          ...
          return {
            ...
            assignedTo: status === 'new' ? null : officers[Math.floor(Math.random() * officers.length)],
  ...
  const filteredReports = reports.filter(report => {
    ...
    if (filters.assignee !== 'all') {
      if (filters.assignee === 'unassigned' && report.assignedTo) {
        return false;
      } else if (filters.assignee !== 'unassigned' && report.assignedTo !== filters.assignee) {
        return false;
      }
    }
