...
contract KavachReporting {
    ...
    // DID (Decentralized Identifier) related mappings
    mapping(bytes32 => string) public phoneHashToDID;
    mapping(string => string) public didToIPFSCID;
    mapping(string => bool) public registeredDIDs;
    string[] public allDIDs; // To track all registered DIDs
    ...
    event UserRegistered(bytes32 indexed phoneHash, string did);
    ...
    /**
     * @dev Register a user with their phone hash and DID
     * @param phoneHash Hash of the user's phone number
     * @param did Decentralized Identifier for the user
     * @param ipfsCID IPFS CID of the encrypted user metadata
     */
    function registerUser(bytes32 phoneHash, string memory did, string memory ipfsCID) external {
        require(!registeredDIDs[did], "DID already registered");
        
        phoneHashToDID[phoneHash] = did;
        didToIPFSCID[did] = ipfsCID;
        registeredDIDs[did] = true;
        allDIDs.push(did);
        
        emit UserRegistered(phoneHash, did);
    }
    
    /**
     * @dev Get all registered DIDs
     * @return All DIDs registered in the system
     */
    function getAllDIDs() external view onlyAuthorized returns (string[] memory) {
        return allDIDs;
    }
    
    /**
     * @dev Get IPFS CID for a specific DID
     * @param did The DID to lookup
     * @return The IPFS CID associated with the DID
     */
    function getIPFSCID(string memory did) external view onlyAuthorized returns (string memory) {
        require(registeredDIDs[did], "DID not registered");
        return didToIPFSCID[did];
    }
